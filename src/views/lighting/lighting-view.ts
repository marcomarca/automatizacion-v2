    ...
    return html`
      <section>
        <header>
          ...
          <p>Supervisión centralizada y control de todas las luminarias.</p>
        </header>

        <div>
          <button type="button" @click=${() => deviceStore.turnAllOn()}>Encender Todas</button>
          <button type="button" @click=${() => deviceStore.turnAllOff()}>Apagar Todas</button>
        </div>
