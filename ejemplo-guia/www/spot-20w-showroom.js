// Spot 20W Showroom v1.0.4
// Custom panel de Home Assistant derivado directamente de panel-backlit-showroom v1.2.0.
// Perfil real: muestra "Spots" del showroom (4 circuitos), configurable desde panel_custom.

const SPOT_20W_DEFAULTS = Object.freeze({
  entities: [
  ...
  officeOnImage: "/local/spot-20w/eg-spot20w-on.png",
  officeOffImage: "/local/spot-20w/eg-spot20w-off.png",
  panelOnImage: "/local/spot-20w/spot20w-on.png",
  panelOffImage: "/local/spot-20w/spot20w-off.png",
});

class Spot20wShowroom extends HTMLElement {
  constructor() {
    super();
  ...
  _config() {
    ...
    const requestedEntities = raw.entities || raw.spot_entities || raw.spotEntities;
    const legacyEntity = raw.entity ? [raw.entity] : [];
    const entities = Array.isArray(requestedEntities) && requestedEntities.length
      ? [...new Set(requestedEntities.filter(Boolean).map(String))]
      : legacyEntity.length
        ? legacyEntity.map(String)
        : [...SPOT_20W_DEFAULTS.entities];
    ...
    const sceneControlEntities = Array.isArray(requestedControls) && requestedControls.length
      ...
      : [...SPOT_20W_DEFAULTS.sceneControlEntities];
    ...
    return {
      entities,
      sceneControlEntities,
      officeOnImage: raw.office_on_image || raw.officeOnImage || SPOT_20W_DEFAULTS.officeOnImage,
      officeOffImage: raw.office_off_image || raw.officeOffImage || SPOT_20W_DEFAULTS.officeOffImage,
      panelOnImage: raw.panel_on_image || raw.panelOnImage || SPOT_20W_DEFAULTS.panelOnImage,
      panelOffImage: raw.panel_off_image || raw.panelOffImage || SPOT_20W_DEFAULTS.panelOffImage,
    };
  }

  _state() {
    const entities = this._config().entities;
    if (!entities.length) return "unavailable";

    const states = entities.map((entity) => this._hass?.states?.[entity]?.state || "unavailable");
    if (states.some((state) => ["unavailable", "unknown"].includes(state))) return "unavailable";
  ...
  async _togglePanel() {
    ...
    try {
      if (desired === "on") {
        // Replica la escena de muestra "Spots" del showroom:
        // apaga el resto del grupo exclusivo y deja encendidos los 4 circuitos de spots.
        const targets = new Set(config.entities);
        const offEntities = config.sceneControlEntities.filter((id) => !targets.has(id));
        await this._setEntitiesState(offEntities, "off");
        await this._setEntitiesState(config.entities, "on");
      } else {
        // Apagar desde esta interfaz desactiva únicamente la muestra Spots.
        await this._setEntitiesState(config.entities, "off");
      }
      ...
      this._clearPending();
      this.render();
      console.error("Spot 20W: error de control", error);
    }
  }
  ...
  render() {
    ...
    const stateLabel = unavailable ? "NO DISPONIBLE" : switching ? "CAMBIANDO…" : state === "mixed" ? "PARCIAL" : visuallyOn ? "ENCENDIDO" : "APAGADO";
    const actionLabel = unavailable ? "Circuito no disponible" : visuallyOn ? "Apagar spots" : "Encender spots";
    ...
    this.shadowRoot.innerHTML = `
      ...
      <main class="layout ${visuallyOn ? "" : "lights-off"}">
        <section class="office-image" aria-label="Spots LED de empotrar instalados en cocina">
          ...
          <div class="office-image__caption"><span>ILUMINACIÓN PROFESIONAL</span><strong>Spot de Empotrar 20W</strong></div>
        </section>

        <section class="product-card" aria-label="Descripción y control del spot LED de 20W">
          <div class="topline">
            <div class="warranty">
          ...
          <div class="product-intro">
            ...
            >
              <img src="${this._escape(visuallyOn ? config.panelOnImage : config.panelOffImage)}" alt="Spot LED de empotrar 20W">
            </button>
            <div class="intro-copy">
              <h1>SPOT DE</h1>
              <p>EMPOTRAR <b>20W</b></p>
              <div class="interaction-copy">Circuito Showroom · <strong>Spots</strong></div>
              ${this._lastError ? `<div class="error-copy">${this._escape(this._lastError)}</div>` : ""}
            </div>
          </div>

          <ul class="reference-specs" aria-label="Características del spot">
            <li><span><svg viewBox="0 0 28 28"><path d="m7 12 5 4 8-4-5-3 5 4-5 3"/></svg></span>20W</li>
            <li><span><svg viewBox="0 0 28 28"><path d="M4 14h20M8 9l-5 5 5 5M20 9l5 5-5 5"/></svg></span>141 x 132 mm</li>
            <li><span><svg class="mint" viewBox="0 0 28 28"><path d="M6 17a8 8 0 0 1 16 0"/></svg></span>3000K</li>
            <li><span><svg viewBox="0 0 28 28"><text class="letters" x="14" y="17">CRI</text></svg></span>CRI&gt;80</li>
            <li><span><svg viewBox="0 0 28 28"><path d="M7 7h14v14H7zM11 7v14M15 7v14M19 7v14M7 11h14M7 15h14M7 19h14"/></svg></span>LED EPISTAR</li>
            <li><span><svg viewBox="0 0 28 28"><path d="M7 7h15l-3 14H4z"/></svg></span>ALUMINIO</li>
            <li class="install"><span><svg viewBox="0 0 28 28"><path d="m8 8 12 12M20 8 8 20M6 9l3-3 3 3-3 3zM16 19l3-3 3 3-3 3z"/></svg></span><span>DE EMPOTRAR</span></li>
            <li><span><svg viewBox="0 0 28 28"><path d="M6 21 14 7l8 14M9 18h10"/></svg></span>ÁNGULO 45°</li>
          </ul>
        </section>
      </main>
    `;
...
if (!customElements.get("spot-20w-showroom")) {
  customElements.define("spot-20w-showroom", Spot20wShowroom);
}
