// Slim Showroom v1.0.5
// Custom panel de Home Assistant derivado directamente de panel-backlit-showroom v1.2.0.
// Estado real: switch.interruptor_inteligente_2_switch_3 (muestra Slims, configurable desde panel_custom).

const SLIM_DEFAULTS = Object.freeze({
  ...
  officeOnImage: "/local/slims/slim-office-on.png",
  officeOffImage: "/local/slims/slim-office-off.png",
  panelOnImage: "/local/slims/slim-product-on.png",
  panelOffImage: "/local/slims/slim-product-off.png",
});

class SlimShowroom extends HTMLElement {
  constructor() {
    super();
  ...
  _config() {
    ...
    const entity = String(raw.entity || SLIM_DEFAULTS.entity);
    ...
    const sceneControlEntities = Array.isArray(requestedControls) && requestedControls.length
      ...
      : [...SLIM_DEFAULTS.sceneControlEntities];
    ...
    return {
      entity,
      sceneControlEntities,
      officeOnImage: raw.office_on_image || raw.officeOnImage || SLIM_DEFAULTS.officeOnImage,
      officeOffImage: raw.office_off_image || raw.officeOffImage || SLIM_DEFAULTS.officeOffImage,
      panelOnImage: raw.panel_on_image || raw.panelOnImage || SLIM_DEFAULTS.panelOnImage,
      panelOffImage: raw.panel_off_image || raw.panelOffImage || SLIM_DEFAULTS.panelOffImage,
    };
  }
  ...
  async _togglePanel() {
    ...
    try {
      if (desired === "on") {
        // Replica la escena de muestra "Slims" del showroom:
        // apaga el resto del grupo exclusivo y deja encendido solo Slims.
        ...
        // Apagar desde esta interfaz desactiva la muestra Slims.
      ...
      this._clearPending();
      this.render();
      console.error("Slims: error de control", error);
    }
  }
  ...
  render() {
    ...
    const actionLabel = unavailable ? "Circuito no disponible" : visuallyOn ? "Apagar slims" : "Encender slims";
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        @media (min-width:981px) and (max-height:720px) {
          ...
          .panel-toggle img { width: clamp(165px,16vw,250px); max-height: 16vh; }
      ...
      <main class="layout ${visuallyOn ? "" : "lights-off"}">
        <section class="office-image" aria-label="Luminarias Slim LED instaladas en un espacio industrial">
          ...
          <div class="office-image__caption"><span>ILUMINACIÓN PROFESIONAL</span><strong>Slims LED empotrables</strong></div>
        </section>

        <section class="product-card" aria-label="Descripción y control de luminarias Slim LED">
          <div class="topline">
            <div class="warranty">
          ...
          <div class="product-intro">
            ...
            >
              <img src="${this._escape(visuallyOn ? config.panelOnImage : config.panelOffImage)}" alt="Luminaria Slim LED 48W 600 x 50 mm">
            </button>
            <div class="intro-copy">
              <h1>SLIM LED</h1>
              <p>600 × 50 mm <b>48W</b></p>
              <div class="interaction-copy">Circuito Showroom · <strong>Slims</strong></div>
              ${this._lastError ? `<div class="error-copy">${this._escape(this._lastError)}</div>` : ""}
            </div>
          </div>

          <ul class="reference-specs" aria-label="Características de la luminaria Slim LED">
            <li><span><svg viewBox="0 0 28 28"><path d="m7 12 5 4 8-4-5-3 5 4-5 3"/></svg></span>48W</li>
            <li><span><svg viewBox="0 0 28 28"><path d="M4 14h20M8 9l-5 5 5 5M20 9l5 5-5 5"/></svg></span>600 x 50mm</li>
            <li><span><svg viewBox="0 0 28 28"><text class="letters" x="14" y="17">PF</text></svg></span>PF&gt;0.9</li>
            <li><span><svg class="mint" viewBox="0 0 28 28"><path d="M6 17a8 8 0 0 1 16 0"/></svg></span>3500K · 1790 Lm</li>
            <li><span><svg class="mint" viewBox="0 0 28 28"><path d="M6 17a8 8 0 0 1 16 0"/></svg></span>4500K · 2206 Lm</li>
            <li><span><svg class="mint" viewBox="0 0 28 28"><path d="M6 17a8 8 0 0 1 16 0"/></svg></span>6500K · 2245 Lm</li>
...
if (!customElements.get("slim-showroom")) {
  customElements.define("slim-showroom", SlimShowroom);
}
