    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        :host {
          ...
          --primary-medium: rgba(255, 255, 255, 0.14);
          --primary-border: rgba(255, 255, 255, 0.28);
          --primary-glow: none;
          ...
          --radius-lg: 10px;
          --radius-pill: 9999px;
          --motion: 140ms cubic-bezier(0.16, 1, 0.3, 1);
        ...
        :host([data-theme="light"]) {
          ...
          --primary-medium: rgba(0, 0, 0, 0.10);
          --primary-border: rgba(0, 0, 0, 0.22);
          --primary-glow: none;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        h1 { font-size: 16px; font-weight: 600; line-height: 1.2; letter-spacing: -0.02em; }
        h2 { font-size: 14px; font-weight: 600; line-height: 1.25; letter-spacing: -0.015em; }
        h3 { font-size: 13px; font-weight: 600; line-height: 1.3; }
        p { color: var(--text-secondary); }

        .app-shell { min-height: 100vh; background: var(--base); }
        .topbar {
          position: sticky;
          top: 0;
          z-index: 50;
        ...
        .topbar-meta { display: flex; align-items: center; justify-content: flex-end; margin-left: auto; gap: 8px; }
        .theme-button {
          width: 32px;
          height: 32px;
          flex: 0 0 32px;
        ...
        .theme-button:hover { background: var(--surface-hover); border-color: var(--border-emphasis); color: var(--text-primary); }
        ...
        .theme-icon-sun, .theme-icon-moon { transform-origin: center; transition: opacity 200ms, transform 200ms; }
        ...
        .dashboard { width: min(1400px, 100%); margin: 0 auto; padding: clamp(12px, 1.8vw, 20px); }
        .surface {
          min-width: 0;
          border: 1px solid var(--border-default);
        ...
        .overview-grid { display: grid; grid-template-columns: 1fr; gap: 12px; margin-top: 4px; }
        .hero-card {
          min-height: 56px;
          padding: 12px 16px;
          display: grid;
        ...
        .hero-copy { min-width: 0; display: flex; align-items: center; gap: 12px; }
        .geist-badge {
          display: inline-flex;
          align-items: center;
          gap: 6px;
        ...
        .primary-grid { display: grid; grid-template-columns: repeat(12, minmax(0, 1fr)); gap: 12px; margin-top: 12px; align-items: start; }
        .scenes-card { grid-column: 1 / -1; padding: 14px 16px; }
        ...
        .general-card { grid-column: 1 / -1; padding: 14px 16px; }
        .system-card { grid-column: span 4; padding: 14px 16px; }
        .activity-card { grid-column: span 8; padding: 14px 16px; }
        .media-card { grid-column: 1 / -1; padding: 14px 16px; }

        .section-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
        .compact-heading { margin-bottom: 12px; }
        .section-heading h2 { margin: 0; font-size: 14px; font-weight: 600; letter-spacing: -0.01em; color: var(--text-primary); }
        .eyebrow {
          font-size: 11px;
          font-weight: 500;
          color: var(--text-tertiary);
        ...
        .device.is-error { border-color: rgba(238, 0, 0, 0.4); }
        .device.is-pending { animation: pulse 1s ease-in-out infinite alternate; }
        ...
        .device-copy strong { display: block; font-size: 13px; font-weight: 500; color: var(--text-primary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .device-copy small { display: block; margin-top: 1px; color: var(--text-secondary); font-size: 11px; }
        .device.is-on .device-copy small { color: var(--text-primary); font-weight: 500; }
        .device-switch {
          width: 28px;
          height: 16px;
          padding: 2px;
        ...
        .device.is-on .device-switch {
          border-color: var(--text-primary);
          background: var(--text-primary);
        }
        .device.is-on .device-switch i {
        ...
        .clear-scene-button:hover:not(:disabled) {
          border-color: var(--error);
          background: rgba(238, 0, 0, 0.08);
          color: var(--error);
        }
        .clear-scene-button .icon { width: 13px; height: 13px; }
        ...
        .scene strong { font-size: 13px; font-weight: 500; color: var(--text-primary); display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .scene small { margin-top: 2px; color: var(--text-secondary); font-size: 11px; display: block; }
        .scene-state {
          min-height: 22px;
          padding: 0 8px;
          display: inline-flex;
        ...
        .general-action:hover:not(:disabled) { background: var(--surface-hover); border-color: var(--border-emphasis); }
        .general-action > span {
          width: 28px;
          height: 28px;
          border-radius: var(--radius-sm);
        ...
        .general-action strong { font-size: 13px; font-weight: 500; color: var(--text-primary); }
        .general-action.power-on > span { color: var(--success); background: rgba(16, 185, 129, 0.1); }
        .general-action.power-off > span { color: var(--error); background: rgba(238, 0, 0, 0.1); }
        .isolated-control { margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-default); }
        .isolated-label { display: block; margin-bottom: 6px; color: var(--text-tertiary); font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: .05em; }

        .media-body { min-height: 64px; display: grid; grid-template-columns: 48px minmax(0, 1fr); gap: 12px; align-items: center; }
        .media-art { width: 48px; height: 48px; border-radius: var(--radius-md); background: var(--surface-control); border: 1px solid var(--border-default); color: var(--text-primary); display: grid; place-items: center; }
        .media-art .icon { width: 22px; height: 22px; }
        .media-copy strong { font-size: 13px; font-weight: 600; color: var(--text-primary); }
        .media-copy span { margin-top: 2px; color: var(--text-secondary); font-size: 11px; }
        .media-copy small { margin-top: 4px; color: var(--text-tertiary); font-size: 10px; }
        .media-state { padding: 3px 8px; border: 1px solid var(--border-default); border-radius: var(--radius-pill); color: var(--text-secondary); font-size: 10px; font-weight: 500; }
        .media-state.is-playing { color: var(--success); border-color: rgba(16, 185, 129, 0.3); background: rgba(16, 185, 129, 0.08); }
        .media-controls { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin-top: 12px; }
        .media-button { min-height: 36px; display: grid; place-items: center; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); color: var(--text-primary); cursor: pointer; transition: background var(--motion), border-color var(--motion); }
        .media-button:hover:not(:disabled) { background: var(--surface-hover); border-color: var(--border-emphasis); }
        .media-button.primary { background: var(--text-primary); color: var(--base); border-color: transparent; }

        .energy-shell { grid-column: span 8; padding: 16px 18px; }
        ...
        .energy-heading-icon { width: 32px; height: 32px; flex: 0 0 auto; display: grid; place-items: center; border-radius: var(--radius-sm); background: var(--surface-control); color: var(--text-primary); }
        .energy-heading-icon .icon { width: 18px; height: 18px; }
        .energy-current { min-width: 160px; padding: 10px 12px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); text-align: right; }
        .energy-current small { color: var(--text-tertiary); font-size: 10px; font-weight: 500; text-transform: uppercase; letter-spacing: .05em; }
        .energy-current strong { margin-top: 2px; font-size: 18px; font-weight: 600; font-variant-numeric: tabular-nums; color: var(--text-primary); }
        .energy-current span { margin-top: 2px; color: var(--text-tertiary); font-size: 10px; }
        .energy-toolbar { margin-top: 12px; display: flex; align-items: center; justify-content: space-between; gap: 8px; }
        .energy-tabs { display: inline-flex; padding: 2px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); }
        .energy-tab { min-width: 54px; height: 26px; padding: 0 10px; border: 0; border-radius: var(--radius-sm); background: transparent; color: var(--text-secondary); cursor: pointer; font-size: 11px; font-weight: 500; transition: background var(--motion), color var(--motion); }
        .energy-tab.is-active { background: var(--surface); color: var(--text-primary); border: 1px solid var(--border-subtle); box-shadow: 0 1px 2px var(--shadow); }
        .energy-day-nav { display: inline-flex; align-items: center; gap: 2px; padding: 2px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); }
        .energy-day-nav button { height: 26px; border: 0; border-radius: var(--radius-sm); background: transparent; color: var(--text-secondary); cursor: pointer; font-size: 11px; font-weight: 500; }
        .energy-day-nav button:hover:not(:disabled) { color: var(--text-primary); background: var(--surface-hover); }
        .energy-day-step { width: 26px; font-size: 14px !important; }
        .energy-day-current { min-width: 54px; padding: 0 8px; color: var(--text-primary) !important; font-weight: 600 !important; }
        .energy-summary { margin-top: 12px; display: grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 8px; }
        .energy-stat { min-width: 0; padding: 12px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); }
        .energy-stat small { display: block; color: var(--text-tertiary); font-size: 10px; font-weight: 500; text-transform: uppercase; letter-spacing: .05em; }
        .energy-stat strong { display: block; margin-top: 4px; font-size: 18px; font-weight: 600; font-variant-numeric: tabular-nums; color: var(--text-primary); }
        .energy-stat span { display: block; margin-top: 2px; color: var(--text-tertiary); font-size: 10px; }
        .energy-note { margin-top: 8px; padding: 8px 10px; border: 1px solid var(--border-default); border-radius: var(--radius-md); color: var(--text-tertiary); background: var(--surface-control); font-size: 10px; line-height: 1.4; }
        .energy-chart-card { margin-top: 8px; padding: 12px 10px 6px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); }
        .energy-chart-title { display: flex; align-items: center; justify-content: space-between; gap: 8px; padding: 0 2px 8px; }
        .energy-chart-title strong { font-size: 12px; font-weight: 600; color: var(--text-primary); }
        .energy-chart-title span { color: var(--text-tertiary); font-size: 10px; }
        .energy-chart-layout { width: 100%; display: flex; min-width: 0; align-items: stretch; }
        .energy-y-axis { position: relative; z-index: 2; flex: 0 0 46px; height: 280px; border-right: 1px solid var(--border-default); background: var(--surface-control); }
        .energy-y-unit { position: absolute; top: 2px; left: 4px; color: var(--text-tertiary); font-size: 9px; font-weight: 600; }
        .energy-y-tick { position: absolute; right: 6px; transform: translateY(-50%); color: var(--text-tertiary); font-size: 9px; font-weight: 500; }
        ...
        .energy-grid-line { stroke: var(--border-default); stroke-width: 1; }
        .energy-axis-text { fill: var(--text-tertiary); font-family: inherit; font-size: 9px; font-weight: 500; }
        .energy-current-label { fill: var(--text-primary); font-family: inherit; font-size: 8.5px; font-weight: 600; text-transform: uppercase; }
        .energy-bar { fill: #888888; opacity: .8; transition: opacity var(--motion); }
        .energy-bar.is-partial { opacity: 1; stroke: var(--text-primary); stroke-width: 1; stroke-dasharray: 3 3; }
        .energy-bar-group:hover .energy-bar { fill: var(--text-primary); opacity: 1; }

        .system-grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 8px; }
        .system-tile { min-height: 52px; padding: 8px 10px; display: grid; grid-template-columns: 30px minmax(0,1fr); gap: 8px; align-items: center; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface); }
        .system-icon { width: 30px; height: 30px; border-radius: var(--radius-sm); background: var(--surface-control); color: var(--text-secondary); display: grid; place-items: center; }
        .system-tile.good .system-icon { color: var(--success); background: rgba(16, 185, 129, 0.1); }
        .system-tile.warning .system-icon { color: var(--warning); background: rgba(245, 166, 35, 0.1); }
        .system-tile.danger .system-icon { color: var(--error); background: rgba(238, 0, 0, 0.1); }
        .system-tile small { color: var(--text-tertiary); font-size: 10px; font-weight: 500; }
        .system-tile strong { margin-top: 2px; font-size: 12px; font-weight: 600; color: var(--text-primary); }
        ...
        .dialog-card { width: min(400px, 100%); padding: 22px; border: 1px solid var(--border-active); border-radius: var(--radius-lg); background: var(--modal); box-shadow: 0 20px 50px var(--shadow-strong); text-align: center; }
        .dialog-icon { width: 44px; height: 44px; margin: 0 auto 12px; display: grid; place-items: center; border-radius: 50%; }
        .dialog-icon.is-power-off { background: rgba(238, 0, 0, 0.1); color: var(--error); }
        .dialog-icon.is-power-on { background: rgba(16, 185, 129, 0.1); color: var(--success); }
        .dialog-card h2 { margin: 8px 0 6px; font-size: 18px; font-weight: 600; color: var(--text-primary); }
        .dialog-card p { margin: 0; color: var(--text-secondary); font-size: 12px; line-height: 1.5; }
        .dialog-card code { color: var(--text-primary); font-size: 11px; }
        .dialog-actions { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; margin-top: 18px; }
        .dialog-actions button { min-height: 40px; border-radius: var(--radius-md); font-size: 13px; font-weight: 600; cursor: pointer; }
        .secondary-button { border: 1px solid var(--border-default); background: transparent; color: var(--text-primary); }
        .secondary-button:hover { background: var(--surface-hover); }
        .primary-button { border: 0; background: var(--text-primary); color: var(--base); }
        .primary-button.confirm-on { background: var(--success); color: #ffffff; }
        .toast { position: fixed; right: 20px; bottom: 20px; z-index: 110; max-width: min(360px, calc(100vw - 32px)); padding: 10px 14px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background: var(--surface-control); box-shadow: 0 12px 30px var(--shadow-strong); font-size: 12px; font-weight: 600; color: var(--text-primary); }
        ...
        @media (max-width: 768px) {
          ...
          .hero-card { min-height: 0; padding: 12px 14px; grid-template-columns: 1fr !important; gap: 10px; }
          .hero-copy { border-bottom: 1px solid var(--border-default); padding-bottom: 8px; }
          .hero-status { justify-content: space-between; width: 100%; align-items: center; flex-direction: row; gap: 10px; }
        ...
        <main class="dashboard">
          <section class="overview-grid">
            <article class="surface hero-card">
              <div class="hero-copy">
                <span class="geist-badge"><i class="badge-dot"></i> SHOWROOM</span>
                <h1>${this._escape(config.title)} Witmind</h1>
              </div>
...
export class ShowroomV2Panel extends ShowroomPanel {}

if (!customElements.get("showroom-v2-panel")) {
  customElements.define("showroom-v2-panel", ShowroomV2Panel);
}

export default ShowroomV2Panel;
